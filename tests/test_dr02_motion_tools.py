...
def test_dr02_xml_loads():
    ...
    model = mujoco.MjModel.from_xml_path("assets/robots/dr02/mjcf/dr02_pos.xml")
    assert model.nq == 36
    assert model.nv == 35
    assert model.nu == 29
...
def test_dr02_motion_tools_fake_motion(tmp_path):
    ...
    assert metrics["joint_pos"].shape == (30, 29)
    assert dataset["joint_vel"].shape == (30, 29)
